...
figure()
 hold on;
output=load('G:\khizer\hyperspectral_datasets\E_CEM-for-Hyperspectral-Target-Detection-master\MF_salinas_c6_pca_output.mat')
 ...
 output=load('G:\khizer\hyperspectral_datasets\E_CEM-for-Hyperspectral-Target-Detection-master\CEM_salinas_c6_pca_output.mat')
 Calc=cal_AUC(output.output(:),logical(output.groundtruth(:)),1,1)
 [PD1_CEM,PF1_CEM,tau1_CEM]=plt_3DROC(output.output(:),logical(output.groundtruth(:)),'CEM',1);
 
  output=load('G:\khizer\hyperspectral_datasets\E_CEM-for-Hyperspectral-Target-Detection-master\ACE_salinas_c6_pca_output.mat')
 Calc=cal_AUC(output.output(:),logical(output.groundtruth(:)),1,1)
 [PD1_ACE,PF1_ACE,tau1_ACE]=plt_3DROC(output.output(:),logical(output.groundtruth(:)),'ACE',1);
 
  output=load('G:\khizer\hyperspectral_datasets\E_CEM-for-Hyperspectral-Target-Detection-master\E-CEM_salinas_c6_pca_output.mat')
 Calc=cal_AUC(output.output(:),logical(output.groundtruth(:)),1,1)
 [PD1_E_CEM,PF1_E_CEM,tau1_E_CEM]=plt_3DROC(output.output(:),logical(output.groundtruth(:)),'E-CEM',1);
 
   output=load('E:\Compressed\hCEM_Demo\salinas_c6_pca_hcem.mat')
 ...
 %[PD1_HTD_NET,PF1_HTD_NET,tau1_HTD_NET]=plt_3DROC(output.output(:),logical(output.groundtruth(:)),'HTD-Net',1);
     ...
     output=load('G:\khizer\HSI-detection-main\salinas_c6_pca_dmbdl.mat')
 ...
 [PD1_BLTSC,PF1_BLTSC,tau1_BLTSC]=plt_3DROC(output.B(:),logical(output.map(:)),'BLTSC',1);
       ...
       output=load('F:\BLTSC-master\New folder\salinas_c6_proposed_pca20_huber.mat')
 Calc=cal_AUC(output.B(:),logical(output.map(:)),1,1)
 ...
 saveas(figure(1),'salinas_pd_pf.png')
 ...
 xlabel('Threshold')
 legend('MF','CEM','ACE','E-CEM','h-CEM','DM-BDL','BLTSC','Proposed')
 saveas(figure(2),'salinas_pd_tau.png')
 ...
 xlabel('Threshold')
 legend('MF','CEM','ACE','E-CEM','h-CEM','DM-BDL','BLTSC','Proposed')
 saveas(figure(3),'salinas_pf_tau.png')